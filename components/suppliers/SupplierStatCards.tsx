type SupplierStatCardsProps = {
  total: number | string
  ddpCount: number | string
  machineCount: number | string
  newThisMonth: number | string
}

export default function SupplierStatCards({
  total,
  ddpCount,
  machineCount,
  newThisMonth,
}: SupplierStatCardsProps) {
  const stats = [
    { label: 'TOTAL SUPPLIERS', value: total, trend: '+6%', trendUp: true },
    { label: 'DDP PARTNERS', value: ddpCount, trend: '+3', trendUp: true },
    { label: 'ACTIVE MACHINES', value: machineCount, trend: '+11%', trendUp: true },
    { label: 'NEW THIS MONTH', value: newThisMonth, trend: '-1', trendUp: false },
  ]
